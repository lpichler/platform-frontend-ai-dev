...
def classify_gh(pr, last_addressed=""):
    ...
    if not JIRA_TITLE_RE.search(pr.get("title") or ""):
        issues.append("missing_jira_key")
    for rv in pr.get("reviews") or []:
        rstate = rv.get("state", "")
        submitted = rv.get("submittedAt") or ""
        if last_addressed and submitted and not timestamp_is_after(submitted, last_addressed):
            continue
...
def has_new_feedback(enriched):
    ...
    for c in enriched["pr_comments"]:
        if is_bot_author(c.get("a", "?")):
            continue
        ct = c.get("t", "")
        if timestamp_is_after(ct, last_addr):
            return True
    return False
...
def fmt_task(enriched):
    ...
    for p in enriched["prs"]:
        ...
        if p["data"].get("title"):
            lines.append(f"    title: {p['data']['title']}")
    ...
    for e in enriched:
        ...
        if "merged" in issues:
            ...
            if ci_only and e["task"].get("last_addressed") and not has_new_feedback(e):
                ...
                ci_fail.append(e)
        elif "missing_jira_key" in issues:
            feedback.append(e)
