...
def timestamp_is_after(value, since):
    """Compare ISO timestamps without losing seconds or timezone information."""
    if not value:
        return False
    if not since:
        return True
    ...
    parsed_since = parse(since)
    if parsed_value is not None and parsed_since is not None:
...
def fmt_comments(comments, label, since=None, max_comments=30):
    ...
    if not comments:
        return f"  {label}: (none)"
    if since:
        comments = [c for c in comments if timestamp_is_after(c.get("t", c.get("created", "")), since)]
    if not comments:
