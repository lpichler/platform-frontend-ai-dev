...
Every GitHub PR created or maintained by this workflow MUST contain its Jira key in square brackets in the title, using this format:

```
feat: add new feature [RHCLOUD-12345]
```

Before opening a PR, use the Jira key for the task and make the title end with `[<JIRA-KEY>]`. If an existing PR is missing the bracketed key, correct it with `gh pr edit` before continuing. Apply the equivalent title update for GitLab MRs. Never invent a key: the task's Jira key is the source of truth.
...
10. **Push + PR**: `git push origin bot/<KEY>`
    ...
    **IMPORTANT**: Do NOT use `gh pr create` / `glab mr create` — don't work in this env. Use API calls:
    ...
    GH (fork): `gh api repos/<upstream-owner>/<repo>/pulls -X POST -f title="... [<KEY>]" -f body="..." -f head="<fork-owner>:bot/<KEY>" -f base="<default-branch>"`
    GH (direct): `gh api repos/<owner>/<repo>/pulls -X POST -f title="... [<KEY>]" -f body="..." -f head="bot/<KEY>" -f base="<default-branch>"`
