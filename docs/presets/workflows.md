...
### PR title and Jira traceability

GitHub PR titles must contain the source Jira key in square brackets, for example:
`feat: add new feature [RHCLOUD-12345]`. The Jira-driven workflow uses the task's
key when opening or repairing a PR title. The PR-label workflow creates a Jira Task
when a labeled PR has no bracketed key, updates the title with the new key, and
transitions that issue to **Release Pending** with a merge comment after the PR is merged.
