...
def _make_pr(state="OPEN", mergeable="MERGEABLE", reviews=None, checks=None, review_decision=None):
    pr = {
        ...
        "title": "feat: thing [RHCLOUD-12345]",
    }
    if review_decision:
...
def test_clean_pr():
    state, issues = classify_gh(_make_pr() | {"title": "feat: thing [RHCLOUD-12345]"})
    assert state == "OPEN"
    assert issues == []


def test_missing_jira_key_is_actionable():
    state, issues = classify_gh(_make_pr() | {"title": "feat: thing"})
    assert "missing_jira_key" in issues
...
def test_review_posted_in_same_minute_as_last_addressed_is_kept():
    reviews = [
        {
            "state": "CHANGES_REQUESTED",
            "author": {"login": "alice"},
            "submittedAt": "2026-07-03T07:37:01Z",
        }
    ]
    state, issues = classify_gh(_make_pr(reviews=reviews), last_addressed="2026-07-03T07:37:00+00:00")
    assert "review:alice" in issues
